...
///
    ...
    internal class NwdiagAttributes : Dictionary<string, string>
    {
        ...
        public NwdiagAttributes(IDictionary<string, string> dic) : base(dic)
        { }

        public NwdiagAttributes() : base()
        { }
    ...
    public class NwdiagRunner
    {
        ...
        private readonly static NwdiagAttributes DefaultAttributes = new NwdiagAttributes()
        {
            { "relations", "html" },
            {"encoding", "utf8" },
            {"image_format", "svg" }
        ...
        public NwdiagRunner(NwdiagEnvironment mocodoEnvironment)
        {
        ...
        /// <returns></returns>
        private NwdiagAttributes GetIdentifiedAttrs(in List<KeyValuePair<string, string>> attributes, string inputFile)
        {
            if ((attributes is null) || (attributes.Count == 0))
            {
                NwdiagAttributes attr = new NwdiagAttributes(DefaultAttributes);
                attr.Add("input", inputFile);
                return attr;
            }

            NwdiagAttributes identifiedAttributes = new NwdiagAttributes();
        ...
        private void GenerateSvgDiagram(in NwdiagAttributes attr)
        {
            string initialFile = attr["input"];
            bool overrideDefaultName = attr.MldNameOverridesMcdName;
            NwdiagAttributes svgDiagAttr = new NwdiagAttributes(attr);
        ...
        private void RunMocodoCmd(string args)
        {
            ...
            start.FileName = nwdiagEnvironment.PythonPath;
            start.Arguments = Path.GetFileName(nwdiagEnvironment.NwdiagPath) + " " + args;
        ...
        /// <param name="attr"></param>
        private List<string> GetMocodoFiles(NwdiagAttributes attr)
        {
        ...
        private void FindRelevantFiles(NwdiagAttributes attr, ref List<string> mocodoFiles, string suffixFile, bool imageOnly)
        {
